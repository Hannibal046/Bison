...
class GarchTuple(namedtuple("GarchTuple", ["spot", "variance"])):
...
def generate_garch(
    ...
    white_noise = torch.normal(mean=0,std=1,size = (n_paths, n_steps)) ## changed here
    variance = torch.empty(size = (n_paths, n_steps))
    variance[:, 0] = cast(Tensor, init_state[1])
    resid = torch.empty(size = (n_paths, n_steps))
    log_spot = torch.empty_like(white_noise)
    log_spot[:, 0] = math.log(cast(Tensor, init_state[0])) ## changed here

    for i_step in range(n_steps - 1):
        variance[:, i_step + 1] = omega + alpha * resid[:, i_step]**2 + beta * variance[:,i_step] + gamma * resid[:, i_step]**2 * torch.where(resid[:, i_step] < 0, 1, 0)
        resid[:, i_step + 1] = torch.sqrt(variance[:, i_step + 1]) * white_noise[:, i_step + 1]  
        log_spot[:, i_step + 1] = (
            log_spot[:, i_step]
            + mu
            + resid[:, i_step + 1]
    ...
    return GarchTuple(log_spot.exp(), variance)